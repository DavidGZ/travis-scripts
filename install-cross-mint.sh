#!/bin/sh
# Usage: install-cross-mint.sh <ubuntu repository name> <package name> [<package name>...]

cat <<EOF | sudo apt-key add -
-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: GnuPG v1.4.10 (GNU/Linux)

=Ma9z
-----END PGP PUBLIC KEY BLOCK-----
EOF

echo deb http://vincent.riviere.free.fr/debian/ $1 contrib | sudo tee -a /etc/apt/sources.list
echo deb-src http://vincent.riviere.free.fr/debian/ $1 contrib | sudo tee -a /etc/apt/sources.list

shift

sudo apt-get update -ym
sudo apt-get install -ym $*
